...
export class SignUpFormComponent {
  ...
  firstNameFormControl = new FormControl('',[Validators.required, Validators.pattern('^[a-zA-Z ]*$'), Validators.minLength(2)]);
  lastNameFormControl = new FormControl('',[Validators.required, Validators.pattern('^[a-zA-Z ]*$'), Validators.minLength(2)]);
  dobFormControl = new FormControl('', [Validators.required]);
  emailFormControl = new FormControl('', [Validators.required, Validators.email]);
  phoneNumFormControl = new FormControl('',[Validators.required, Validators.pattern(/^[0-9]+$/), Validators.minLength(7)]);
  passwordFormControl =  new FormControl('',[Validators.required, Validators.pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]+$/) ,Validators.minLength(8)]);
  ...
  onSubmit(): void {
    ...
    if(this.firstNameFormControl.hasError('minLength') || this.firstName.length < 2){
    ...
    if(this.lastNameFormControl.hasError('minLength') || this.lastName.length < 2){
    ...
    if(this.phoneNumFormControl.hasError('minLength') || this.phoneNum.length < 7){
    ...
    if(this.passwordFormControl.hasError('minLength') || this.password.length < 8){
      this.passwordError = "Password must be at least 8 characters long";
      return;
    }
